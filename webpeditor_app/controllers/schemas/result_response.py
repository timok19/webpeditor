...
class BasedResultResponse(Schema, Generic[_TResponse]):
    ...
    @classmethod
    def failure_500(cls, message: str) -> tuple[HTTPStatus, "BasedResultResponse[_TResponse]"]:
        return cls.failure(message, status_code=HTTPStatus.INTERNAL_SERVER_ERROR)

    @classmethod
    def failure(cls, message: str, *, status_code: HTTPStatus) -> tuple[HTTPStatus, "BasedResultResponse[_TResponse]"]:
    ...
    @classmethod
    def from_value(cls, value: _TResponse) -> tuple[HTTPStatus, "BasedResultResponse[_TResponse]"]:
        return HTTPStatus.OK, cls(value=value)

    @classmethod
    def from_failure(cls, failure: FailureContext) -> tuple[HTTPStatus, "BasedResultResponse[_TResponse]"]:
        return cls.__map_status_code(failure.error_code), cls(message=failure.message)

    @classmethod
    def empty(cls) -> tuple[HTTPStatus, "BasedResultResponse[_TResponse]"]:
